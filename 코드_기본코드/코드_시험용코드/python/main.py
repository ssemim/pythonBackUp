...
def bfs(grid, start, target, wall):
    ...
    while queue:
        ...
        for d, (dr, dc) in enumerate(DIRS):
            shot = True
            # 포탄 최대 범위 4까지 상하좌우 탐색
            for m in range(4):
                nr, nc = r + dr * m, c + dc * m
                if 0 <= nr < rows and 0 <= nc < cols:
                    # 만약 전방에 가로막고 있는 바위가 있다면
                    if grid[nr][nc] == 'R':
                        # 발사하지 않고 브레이크
                        shot = False
                        break
                    # 전방에 바위가 없다면
                    if (nr, nc) == target and shot:
                        # 일반 포탄이 있다면 일반 포탄 발사
                        # 만약 일반 포탄이 없다면 메가 포탄 발사
                        if bullet:
                            return actions + [FIRE_CMDS[d]]
                        else:
                            return actions + [MEGA_CMDS[d]]
                
            

        for d, (dr, dc) in enumerate(DIRS):
            nr, nc = r + dr, c + dc
            # 물 지역이거나 바위 지역이면 갈 수 없다.
            if 0 <= nr < rows and 0 <= nc < cols and grid[nr][nc] not in wall and (nr, nc) not in visited:
                ...
                if grid[nr][nc] == 'T':
                    ...
                    if bullet:
                        ...
                        queue.append(((nr, nc), actions + [MEGA_CMDS[d]]))
                        continue
...
MEGA_CMDS = {0: "R F M", 1: "D F M", 2: "L F M", 3: "U F M"} # 메가 포탄 발사 커맨드
START_SYMBOL = 'M'
TARGET_SYMBOL = 'X'
TREE = 'T'
WALL_SYMBOL = ('W','R') # 갈 수 없는 WALL_SYMBOL에 물 지역 'W' 추가
