...
for test_case in range(1, T + 1):  # 테스트 케이스 돌면서
    ...
    for _ in range(100):  # 한줄씩 받아넣어서 16X16 2차원 행렬을 만듬
        row = list(map(int, input().strip()))
        graph.append(row)

    for i in range(100):
        for j in range(100):  # 2차원 배열 돌면서 시작점 도착점 뽑아내자
            if graph[i][j] == 2:
                si, sj = i, j
            if graph[i][j] == 3:
                ei, ej = i, j
        ...
        while q:
            ...
            if graph[i][j] == 3:  # 도착지 도착! 1반환
                ...
                for d in range(4):  # 상하좌우 무빙치면서
                    ...
                    if 0 <= ni < 100 and 0 <= nj < 100 and graph[ni][nj] != 1 and not visited[ni][nj]:
                        visited[ni][nj] = 1
