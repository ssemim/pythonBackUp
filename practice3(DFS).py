...
# 인접 행렬(adjust matrix)
# 어떤 정점 x와 어떤 정점 y가 연결되어있는지 여부를 True(1) 또는 False(0)으로 나타냄
# True(1) => 연결되어있다, x에서 y로 가는 간선이 존재한다 // adjM[x][y] = True // adjM[x][y] = 1
# False(0) => 연결되어있지 않음, x에서 y로 가는 간선이 없다. // adjM[x][y] = False // adjM[x][y] = 0
...
for i in range(E):
    ...
    adjM[s][e] = 1
    adjM[e][s] = 1  # 화살표가 없어서 그래프에 방향이 없으므로, 반대의 경우도 포함시켜 줄 것


# for i in range(V + 1): # 2차원 행렬 꼴의 그래프를 그려보면
#     print(adjM[i])
